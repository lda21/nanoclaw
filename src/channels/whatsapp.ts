...
registerChannelAdapter('whatsapp', {
  factory: () => {
     ...
     * so the host can stage it into the per-session inbox.
     *
     * The adapter must NOT write the file to disk or invent a `localPath`
     * itself. The canonical flow (shared by every channel) is that
     * `extractAttachmentFiles()` in session-manager.ts takes each attachment
     * carrying base64 `data`, writes it to `inbox/<messageId>/<filename>`
     * (which is mounted into the agent container, with symlink/traversal
     * guards), sets the real container-visible `localPath`, and strips
     * `data`. A previous version wrote the file to the host's global
     * `data/attachments/` and set `localPath: attachments/<file>` directly —
     * but that directory is never mounted into the container, so the agent
     * saw a `localPath` that pointed at nothing, and the host's staging step
     * skipped the attachment (no `data` to write).
     */
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async function downloadInboundMedia(
      ...
      for (const { key, type, ext } of mediaTypes) {
        ...
        try {
          ...
          // host re-validates with isSafeAttachmentName before writing, but we
          // also sanitize here so the name we report is sane.
    ...
    async function connectSocket(): Promise<void> {
      ...
      sock.ev.on('connection.update', (update) => {
        ...
        if (qr && !phoneNumber) {
          // QR code auth — print to terminal AND save as a PNG. The full-size
          // terminal QR often overflows the screen; `small: true` packs two
          // rows per line, and the PNG scans reliably at any zoom.
          (async () => {
            const qrPng = path.join(process.cwd(), 'store', 'whatsapp-qr.png');
            try {
              const QRCode = await import('qrcode');
              const qrText = await QRCode.toString(qr, { type: 'terminal', small: true });
              log.info('WhatsApp QR code — scan with WhatsApp > Linked Devices:\n' + qrText);
              await QRCode.toFile(qrPng, qr, { width: 512, margin: 2 });
              log.info('WhatsApp QR image saved', { path: qrPng });
            } catch (err) {
              log.info('WhatsApp QR code (raw)', { qr, err });
            }
          })();
        }
        ...
        if (connection === 'close') {
          ...
          // Reconnect only for transient drops — never during a deliberate
          // shutdown and never after a server-side logout.
          ...
          if (shuttingDown) {
            // Clean process shutdown (SIGTERM → teardown → sock.end()). The
            // socket closes with no status code. The saved session is still
            // valid, so we must NOT touch store/auth here — clearing it on
            // every shutdown forced a fresh QR/pairing on each restart.
            log.info('WhatsApp closed for shutdown — auth preserved for restart');
            ...
            // reason === loggedOut: WhatsApp invalidated the session server-side.
