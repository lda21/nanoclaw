...
import { startCodexGeneration, type CodexGenerationUsage } from '../tracing/index.js';
...
async function* runOneTurn(
  ...
  let resultText = '';
  let turnDone = false;
  // Best-effort token usage from the app-server's turn/completed notification.
  let usage: CodexGenerationUsage | undefined;

  // Langfuse generation span for this turn (no-op when tracing is disabled).
  // The Codex app-server is a separate process the Claude instrumentation can't
  // see, so we record the generation manually. Nests under the poll-loop's
  // active turn trace.
  const generation = startCodexGeneration({ model, input: inputText });
  ...
  const handler = (n: JsonRpcNotification): void => {
    ...
    switch (method) {
      ...
      case 'turn/completed': {
        const u = (params.usage ?? (params.turn as { usage?: unknown } | undefined)?.usage) as
          | { input_tokens?: number; output_tokens?: number; inputTokens?: number; outputTokens?: number; total_tokens?: number }
          | undefined;
        if (u) {
          usage = {
            input: u.input_tokens ?? u.inputTokens,
            output: u.output_tokens ?? u.outputTokens,
  ...
  try {
    ...
    generation.end({ output: resultText || undefined, usage });
    clearTimeout(timer);
